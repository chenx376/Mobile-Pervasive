...
class DataReceiver(WebSocketBaseClient, Thread):
	...
	def received_message(self, msg):
		...
		s = s.replace(",","\n") # Change commas to newlines so file is one number per line (Amelie)
		...
		try:  # In case the file has been closed (user stopped data collection), surround by try-except
            timestamp = datetime.datetime.now().strftime('%Y-%h-%d_%H-%M-%S.%f')
            self.output_file_handle.write(timestamp + "," + s + '\n')
		except Exception as e:
